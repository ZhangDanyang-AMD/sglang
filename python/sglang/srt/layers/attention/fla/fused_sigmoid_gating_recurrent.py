...
def fused_sigmoid_gating_delta_rule_update_kernel(
    ...
    if USE_INITIAL_STATE:
        idx = tl.load(h0_indices + i_n).to(tl.int32)
        if idx >= 0:
            p_h0 = (
                h0_source
                + idx * HV * K * V
            ...
            b_h0 = tl.load(p_h0, mask=mask_h, other=0).to(tl.float32)
            b_h += b_h0
...
def fused_sigmoid_gating_delta_rule_update_kernel_opt(
    ...
    o_h = tl.arange(0, shared_h)

    b_h = tl.zeros([shared_h, BK, BV], dtype=tl.float32)
    if USE_INITIAL_STATE:
        idx = tl.load(h0_indices + i_n)
        if idx >= 0:
            p_h0 = (
                ...
                + idx * HV * K * V
                + (o_k[None, :, None]) * V
                + (o_h[: ,None , None] + i_h * shared_h) * K * V
                + o_v[None, None,:]
            )
    ...
    for _ in range(0, T):
        ...
        # v/o are laid out as [..., HV, V]; stride between rows is V (not BV).
        p_v = v + (bos * HV + i_h * shared_h + o_h)[:, None] * V + o_v[None, :]
        p_o = o + ((i_k * all + bos) * HV + i_h * shared_h + o_h)[:, None] * V + o_v[None, :]

        # Gating computation pointers
        # Gating params are per HV (i.e. per shared_h lane), not per V.
        p_A_log = A_log + i_h * shared_h + o_h
        p_b = b + bos * HV + i_h * shared_h + o_h
        p_a = a + bos * HV + i_h * shared_h + o_h
        p_dt_bias = dt_bias + i_h * shared_h + o_h
        ...
        b_q_i = b_q_i * scale
        # [sh, BV]
        mask_v_2d = (o_h[:, None] >= 0) & mask_v[None, :]
        b_v = tl.load(p_v).to(tl.float32)

        # Apply gating to hidden state: h *= exp(g)
        # [sh, BK, BV] * [sh]
        b_h *= tl.exp(b_g[:, None, None])

        # Delta rule: v -= sum(h * k, dim=0)
        # [sh, BV] = sum_{BK}([sh, BK, BV] * [BK])
        b_v -= tl.sum(b_h * b_k_i[None, :, None], 1)
        ...
        # Update hidden state: h += k[:, None] * v[None, :]
        # [sh, BK, BV] = [BK] * [sh, BV]
        b_h += b_k_i[None, :, None] * b_v[:, None, :]

        # Compute output: o = sum(h * q, dim=0)
        # [sh, BV] = [sh, BK, BV] * [BK]
        b_o = tl.sum(b_h * b_q_i[None, :, None], 1)
        b_o = tl.where(mask_v_2d, b_o, 0.0)
        tl.store(p_o, b_o.to(p_o.dtype.element_ty), mask=mask_v_2d)
...
def fused_sigmoid_gating_delta_rule_update(
    ...
    ms = triton.testing.do_bench(lambda: fused_sigmoid_gating_delta_rule_update_kernel[grid](
        ...
        b=b,
        o=o_test,
        h0_source=initial_state_source_test,
        h0_indices=initial_state_indices,
        cu_seqlens=cu_seqlens,
        scale=scale,
        T=T,
...
@input_guard
def fused_sigmoid_gating_delta_rule_update_opt(
    A_log: torch.Tensor,
    a: torch.Tensor,
    dt_bias: torch.Tensor,
    softplus_beta: float,
