...
def benchmark_kernel(
    ...
    """
    Benchmark the fused sigmoid gating delta rule update kernel.
    ...
    total_params = (
        q.numel() + k.numel() + v.numel() + b.numel() + 
        A_log.numel() + a.numel() + dt_bias.numel()
    )
    if use_initial_state:
        total_params += initial_state_source.numel()
    
    memory_gb = (total_params * q.element_size()) / (1024**3)
...
def main():
    ...
    parser.add_argument("--repeats", type=int, default=20, help="Number of benchmark iterations")
    ...
    if args.sweep:
        ...
        # Sweep over different configurations
        configs = [
            # (batch_size, seq_len, num_heads, head_dim_k, head_dim_v)
            (1, 512, 32, 128, 128),
            (2, 512, 32, 128, 128),
            (4, 512, 32, 128, 128),
            (1, 1024, 32, 128, 128),
        ...
        results = []
        for bs, sl, nh, hdk, hdv in configs:
            try:
                ...
                result["config"] = f"B{bs}_S{sl}_H{nh}_K{hdk}_V{hdv}"
                results.append(result)
            except Exception as e:
                print(f"Error with config B{bs}_S{sl}_H{nh}_K{hdk}_V{hdv}: {e}")
                continue
        ...
        print("="*80)
        print(f"{'Config':<25} {'Avg Time (ms)':<15} {'Throughput (tokens/s)':<25} {'Memory (GB)':<12}")
        print("-"*80)
        for r in results:
            print(f"{r['config']:<25} {r['avg_time_ms']:<15.3f} {r['throughput']:<25.2f} {r['memory_gb']:<12.3f}")
        ...
        # Single benchmark - compare both implementations
        print("\n" + "="*80)
        print("Original kernel benchmark:")
        print("="*80)
        benchmark_kernel(
            func=fused_sigmoid_gating_delta_rule_update,
            batch_size=args.batch_size,
            seq_len=args.seq_len,
            num_heads=args.num_heads_qk,
            head_dim_k=args.head_dim,
        ...
        print("\n" + "="*80)
        print("Optimized kernel benchmark:")
        print("="*80)
        benchmark_kernel(
            func=fused_sigmoid_gating_delta_rule_update_opt,
            batch_size=args.batch_size,
            seq_len=args.seq_len,
            num_heads=args.num_heads_qk,
            head_dim_k=args.head_dim,
